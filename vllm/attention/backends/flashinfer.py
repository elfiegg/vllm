...
class FlashInferMetadata(AttentionMetadata):
    ...
    def begin_forward(self):
        if self.num_prefill_tokens > 0:
            ...
            assert len(self.paged_kv_indptr) == batch_size + 1
            for i in range(batch_size):
                num_blocks_for_request = self.paged_kv_indptr[i + 1] - self.paged_kv_indptr[i]
                print(f"Request {i}: {num_blocks_for_request} blocks")

            print("self.paged_kv_indptr: ", self.paged_kv_indptr.shape)
            print("self.paged_kv_indices: ", self.paged_kv_indices.shape)
            print("self.paged_kv_last_page_len: ", self.paged_kv_last_page_len.shape)
